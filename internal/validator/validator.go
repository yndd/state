...
func ValidateCreate(ce config.ConfigEntry, x interface{}) (ygot.ValidatedGoStruct, error) {
	m := ce.GetModel()
...
func ValidateUpdate(ce config.ConfigEntry, updates []*gnmi.Update, replace, jsonietf bool, origin Origin) error {
	curGoStruct := ce.GetRunningConfig()
	...
	if origin == Origin_Subscription || origin == Origin_GnmiServer {
		if print {
			...
			fmt.Printf("ValidateUpdate origin: %s jsonTree done: \n, %s \n", origin, j)
		}
...
func ValidateDelete(ce config.ConfigEntry, paths []*gnmi.Path, origin Origin) error {
	curGoStruct := ce.GetRunningConfig()
